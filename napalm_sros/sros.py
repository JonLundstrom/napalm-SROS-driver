...
class NokiaSROSDriver(NetworkDriver):
    ...
    def get_arp_table(self, vrf=""):
        ...
        # helper function
        def _get_arp_table(interface_path, discovered_nei):

            arp_table.append(
                {
                    "interface": self._find_txt(
                        interface_path, "state_ns:interface-name", namespaces=self.nsmap
                    ),
                    "mac": self._find_txt(
                        interface_path,
                        f"{nei_path}/state_ns:mac-address",
                        namespaces=self.nsmap,
                    ),
                    "ip": discovered_nei,
                    "age": convert(
        ...
        nei_path = "state_ns:ipv4/state_ns:neighbor-discovery/state_ns:neighbor"

        for interface in result.xpath(
            "state_ns:state/state_ns:router/state_ns:interface", namespaces=self.nsmap
        ):
            discovered_nei_ip = self._find_txt(
                interface, f"{nei_path}/state_ns:ipv4-address", namespaces=self.nsmap,
            )
            if discovered_nei_ip == "":
                continue
            _get_arp_table(interface, discovered_nei_ip)
    ...
    def get_bgp_neighbors(self):
        ...
        def _build_state_neighbor(instance_name, instance_id):
            bgp_neighbors.update(
                {instance_name: {"router-id": instance_id, "peers": {}}}
            )
